...
interface EinkaeuferDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Einkaeufer['fields']) => Promise<void>;
  defaultValues?: Einkaeufer['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function EinkaeuferDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: EinkaeuferDialogProps) {
  const [fields, setFields] = useState<Partial<Einkaeufer['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'einkaeufer');
      await onSubmit(clean as Einkaeufer['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Einkaeufer['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Einkäufer bearbeiten' : 'Einkäufer hinzufügen';
