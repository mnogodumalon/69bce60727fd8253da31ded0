...
interface EinkaufslisteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Einkaufsliste['fields']) => Promise<void>;
  defaultValues?: Einkaufsliste['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function EinkaufslisteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: EinkaufslisteDialogProps) {
  const [fields, setFields] = useState<Partial<Einkaufsliste['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'einkaufsliste');
      await onSubmit(clean as Einkaufsliste['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Einkaufsliste['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Einkaufsliste bearbeiten' : 'Einkaufsliste hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="listenname">Name der Liste</Label>
            <Input
              id="listenname"
              value={fields.listenname ?? ''}
              onChange={e => setFields(f => ({ ...f, listenname: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
