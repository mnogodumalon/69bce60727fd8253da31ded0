...
import { EinkaeuferViewDialog } from '@/components/dialogs/EinkaeuferViewDialog';
...
const EINKAUFSARTIKEL_FIELDS = [
  { key: 'artikelname', label: 'Artikel', type: 'string/text' },
  { key: 'menge', label: 'Menge / Einheit', type: 'string/text' },
  { key: 'liste_ref', label: 'Einkaufsliste', type: 'applookup/select', targetEntity: 'einkaufsliste', targetAppId: 'EINKAUFSLISTE', displayField: 'listenname' },
  { key: 'person_ref', label: 'Zugewiesene Person', type: 'applookup/select', targetEntity: 'einkaeufer', targetAppId: 'EINKAEUFER', displayField: 'vorname' },
  { key: 'erledigt', label: 'Erledigt', type: 'bool' },
];
const EINKAEUFER_FIELDS = [
...
const EINKAUFSLISTE_FIELDS = [
  { key: 'listenname', label: 'Name der Liste', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'datum', label: 'Datum', type: 'date/date' },
];

const ENTITY_TABS = [
  { key: 'einkaufsartikel', label: 'Einkaufsartikel', pascal: 'Einkaufsartikel' },
  { key: 'einkaeufer', label: 'Einkäufer', pascal: 'Einkaeufer' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('einkaufsartikel');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    einkaufsartikel: new Set(),
    einkaeufer: new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'einkaufsartikel': return (data as any).einkaufsartikel as Einkaufsartikel[] ?? [];
      case 'einkaeufer': return (data as any).einkaeufer as Einkaeufer[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'einkaufsartikel':
        lists.einkaufslisteList = (data as any).einkaufsliste ?? [];
        lists.einkaeuferList = (data as any).einkaeufer ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'einkaufsartikel' && fieldKey === 'liste_ref') {
      const match = (lists.einkaufslisteList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.listenname ?? '—';
    }
    if (entity === 'einkaufsartikel' && fieldKey === 'person_ref') {
      const match = (lists.einkaeuferList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'einkaufsartikel': return EINKAUFSARTIKEL_FIELDS;
      case 'einkaeufer': return EINKAEUFER_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'einkaufsartikel': return {
        create: (fields: any) => LivingAppsService.createEinkaufsartikelEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateEinkaufsartikelEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteEinkaufsartikelEntry(id),
      };
      case 'einkaeufer': return {
        create: (fields: any) => LivingAppsService.createEinkaeuferEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateEinkaeuferEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteEinkaeuferEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'einkaufsartikel' || dialogState?.entity === 'einkaufsartikel') && (
        <EinkaufsartikelDialog
          open={createEntity === 'einkaufsartikel' || dialogState?.entity === 'einkaufsartikel'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'einkaufsartikel' ? handleUpdate : (fields: any) => handleCreate('einkaufsartikel', fields)}
          defaultValues={dialogState?.entity === 'einkaufsartikel' ? dialogState.record?.fields : undefined}
          einkaufslisteList={(data as any).einkaufsliste ?? []}
          einkaeuferList={(data as any).einkaeufer ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Einkaufsartikel']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Einkaufsartikel']}
        />
      )}
      {(createEntity === 'einkaeufer' || dialogState?.entity === 'einkaeufer') && (
        <EinkaeuferDialog
          open={createEntity === 'einkaeufer' || dialogState?.entity === 'einkaeufer'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'einkaeufer' ? handleUpdate : (fields: any) => handleCreate('einkaeufer', fields)}
          defaultValues={dialogState?.entity === 'einkaeufer' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Einkaeufer']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Einkaeufer']}
        />
      )}
      ...
      {viewState?.entity === 'einkaufsartikel' && (
        <EinkaufsartikelViewDialog
          open={viewState?.entity === 'einkaufsartikel'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'einkaufsartikel', record: r }); }}
          einkaufslisteList={(data as any).einkaufsliste ?? []}
          einkaeuferList={(data as any).einkaeufer ?? []}
        />
      )}
      {viewState?.entity === 'einkaeufer' && (
        <EinkaeuferViewDialog
          open={viewState?.entity === 'einkaeufer'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'einkaeufer', record: r }); }}
        />
      )}
