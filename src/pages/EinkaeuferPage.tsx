...
import type { Einkaeufer } from '@/types/app';
...
export default function EinkaeuferPage() {
  const [records, setRecords] = useState<Einkaeufer[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Einkaeufer | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Einkaeufer | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Einkaeufer | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getEinkaeufer());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Einkaeufer['fields']) {
    await LivingAppsService.createEinkaeuferEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Einkaeufer['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateEinkaeuferEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteEinkaeuferEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Einkäufer"
      subtitle={`${records.length} Einkäufer im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Einkäufer. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
